...
class MultiPlateGenerator:
    def __init__(self, adr_plate_model, adr_font):
        ...
        # 车牌底板路径
        self.adr_plate_model = os.path.join(script_dir, adr_plate_model)
        # 车牌字符路径
        self.adr_font = os.path.join(script_dir, adr_font)

        # 车牌字符图片，预存处理
        self.font_imgs = {}
        font_filenames = glob(os.path.join(self.adr_font, '*jpg'))
    ...
    def generate_plate(self, enhance=False):
        ...
        if not os.path.exists(plate_model_path):
            print(f"错误：找不到车牌底板文件: {plate_model_path}")
            print(f"当前车牌信息: 号码={plate_number}, 颜色={bg_color}, 双层={is_double}, 高度={height}")
            print(f"可用的底板文件:")
            for file in os.listdir(self.adr_plate_model):
                if file.endswith('.PNG'):
                    print(f"  - {file}")
            raise FileNotFoundError(f"找不到车牌底板文件: {plate_model_path}")
            
        img_plate_model = cv2.imread(plate_model_path)
        if img_plate_model is None:
            raise ValueError(f"无法读取车牌底板文件: {plate_model_path}")
            
        img_plate_model = cv2.resize(img_plate_model, (440 if len(plate_number) == 7 else 480, height))

        font_img_mask = np.zeros_like(img_plate_model)
        font_img_mask[:] = [255,0,0] # 
        ...
        for i in range(len(plate_number)):
            ...
            font_img_mask = copy_to_image_multi(font_img_mask, font_img,
                                                  number_xy[i, :], bg_color, is_red)

        img_plate_model = cv2.blur(img_plate_model, (3, 3))

        return img_plate_model, font_img_mask, number_xy, plate_number, bg_color, is_double
    ...
    def generate_plate_special(self, plate_number, bg_color, is_double, enhance=False):
        ...
        return img_plate_model , font_img


def parse_args():
    parser = argparse.ArgumentParser(description='中国车牌生成器')
    parser.add_argument('--number', default=10, type=int, help='生成车牌数量')
    saved_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plates_base')
    parser.add_argument('--save-adr', default=saved_dir, help='车牌保存路径')
    parser.add_argument('--double', default=False, type=bool, help='是否仅仅生成双层车牌')  
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        # 重置保存目录，删除已存在的数据
        import shutil
        if os.path.exists(args.save_adr):
            shutil.rmtree(args.save_adr)
        args.double = False
    ...
    save_dir = args.save_adr
    for i in tqdm(range(args.number)):
        
        is_enhanced = random_select([False] * 9 + [True])
        
        img, font_img, number_xy, gt_plate_number, bg_color, is_double = generator.generate_plate(is_enhanced)
        
        if args.double:
            while not is_double:
                img, font_img, number_xy, gt_plate_number, bg_color, is_double = generator.generate_plate()
        
        _out_name = f"{gt_plate_number}_{bg_color}_{is_double}"
        target_path = os.path.join(save_dir,"img", f"{_out_name}.jpg")
        mkdir(os.path.join(save_dir,"img"))
        # cv2.imwrite(target_path, img)
        img_pil = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        img_pil.save(target_path)
        
        font_img = font_img.copy()
        # 使用 np.all 来比较每个像素的 RGB 值
        mask = np.all(font_img == [255, 0, 0], axis=-1)
        # 创建RGBA图像,将背景色设为透明
        rgba = np.zeros((font_img.shape[0], font_img.shape[1], 4), dtype=np.uint8)
        rgba[~mask] = np.concatenate([font_img[~mask], np.full((np.sum(~mask), 1), 255, dtype=np.uint8)], axis=1) # 非背景区域设为原始颜色并添加不透明alpha通道
        rgba[mask] = [0, 0, 0, 0]  # 背景区域设为完全透明
        font_img = Image.fromarray(rgba)
        mkdir(os.path.join(save_dir,"fontmask"))
        font_img.save(os.path.join(save_dir,"fontmask", f"{_out_name}.png"))
